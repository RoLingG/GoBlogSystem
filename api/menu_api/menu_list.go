...
func (MenuApi) MenuListView(c *gin.Context) {
	...
	err = global.DB.Preload("ImageModel").Order("sort desc").Find(&menuImages).Select("menu_id in ?", menuIDList).Error //根据菜单的ID去关联的图片表内获取对应的图片的ID和序号
	if err != nil {
		global.Log.Error(err)
	...
	res.OKWithList(menuRes, int64(len(menuRes)), c)
	return
}
