...
type Action struct {
	...
	title       string
	itemList    []string
	model       *LogStashModel // 创建之后赋值给它，用于后期更新
	token       string
	logType     LogType
}
...
func (action *Action) setItem(label string, value any, level LogLevel) {
	// 判断类型
...
func (action *Action) SetFlush() {
	action.level = action.model.LogLevel
	action.save()
}
...
func (action *Action) save() {
	...
	// 更新操作
	global.DB.Model(action.model).Updates(LogStashModel{
		LogLevel: action.level,
		Title:    action.title,
